...
function drawScene() {
	...
	//gl.viewport(0,0,gl.drawinBufferWidth, gl.drawinBufferHeight);
    ...
    /*var plane = Player_Plane();
    plane.rotAngleXX = -60; plane.sx = 0.60; plane.sy = 0.9; plane.sz = 0.6;
    plane.tx = 0; plane.ty = -0.60; plane.tz = -2.25;
    drawModel(plane,
        mvMatrix,
        primitiveType);

    var player = Player_Plane();
    player.rotAngleXX = 0; player.sx = 0.10; player.sy = 0.10; player.sz = 0.10;
    player.tx = tm+velocity; player.ty = -0.80; player.tz = -2.25;
        drawModel(player,
        mvMatrix,
        primitiveType);*/
	...
	for(var j = 0; j < sceneModels.length; j++){
		drawModel( sceneModels[j],
			   mvMatrix,
	           primitiveType );
		sceneModels[j].tz += 0.1;
		sceneModels[j].ty -= 0.05;
		//sceneModels[j].ty -=0.01;
	}
